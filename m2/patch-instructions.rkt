...
(define/contract (patch-instructions p)
  ...
  (define (patch-set fn . cvs)
    (define tmps (current-patch-instructions-registers))
    (define-values (ls params)
      (for/fold ([ls '()] [params '()]) ([cv cvs] [tmp tmps])
        (define check (car cv))
        (define value (cdr cv))
        (if (or (eq? check #t) (check value))
            (values (append-e ls `(set! ,tmp ,value)) (append-e params tmp))
            (values ls (append-e params value)))))
...
(module+ test
  ...
  (check-match
    ...
    ;; any -> boolean
    ;; check if symbol is a register in current-patch-instructions-registers 
